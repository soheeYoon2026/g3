#!/usr/bin/env python3
"""Authenticated HTTP wrapper for the raw-STL DoMINO inference command."""
...
CHECKPOINT = Path(
    os.environ.get(
        ...
        ROOT / "var/domino-automotive-runs/decoder-5epoch.pt",
    )
)
...
app = FastAPI(title="AOX G3 DoMINO Inference", version="0.1.0")


def _model_name() -> str:
    return f"DoMINO automotive decoder fine-tune - {CHECKPOINT.stem}"
...
async def recommend(
    ...
    with tempfile.TemporaryDirectory(prefix="aox-domino-recommend-") as temp_dir:
        ...
        try:
            ...
            command = [
                str(PYTHON), str(RECOMMEND_SCRIPT),
                "--stl", str(stl_path),
            ...
            result = json.loads((output_dir / "recommendations.json").read_text())
    ...
    return {
        ...
        "candidate_count": len(result["candidates"]) - 1,
...
async def infer(
    ...
    if not all((CHECKPOINT.is_file(), SCRIPT.is_file(), PYTHON.is_file())):
        raise HTTPException(status_code=503, detail="DoMINO service files are unavailable")
    if flow_axis not in {"+x", "-x", "+y", "-y", "+z", "-z"}:
        raise HTTPException(status_code=400, detail="invalid flow_axis")
    ...
    with tempfile.TemporaryDirectory(prefix="aox-domino-") as temp_dir:
        ...
        try:
            ...
            raise HTTPException(status_code=504, detail="DoMINO inference timed out") from exc
        except subprocess.CalledProcessError as exc:
            detail = (exc.stderr or exc.stdout or str(exc)).strip().splitlines()[-1]
            raise HTTPException(status_code=422, detail=f"DoMINO inference failed: {detail}") from exc
        except (OSError, ValueError, json.JSONDecodeError) as exc:
            raise HTTPException(status_code=422, detail=f"DoMINO result is invalid: {exc}") from exc
    ...
    return {
        ...
        "flow_axis": result.get("flow_axis"),
